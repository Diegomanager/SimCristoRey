...
public class SimuladorCooperativaService {
    ...
    public void configurar(long msPorMin, int maxSoc, double intv,
        ...
        try {
            ...
            // FIX: si la config trae algo invalido (ej. mapas nulos), no debe
            // tumbar el hilo silenciosamente -- se loguea y se sigue con
            // probabilidades por defecto del generador.
            System.err.println(">>> [Motor] Aviso: no se pudo aplicar la configuracion de probabilidades: " + e);
        }
    }
    ...
    public void reanudar() { if (!corriendo || !pausado) return; pausado = false; synchronized (this) { notifyAll(); } publicar(TipoEvento.SIMULACION_REANUDADA, "Reanudado"); }
    public void detener() { corriendo = false; pausado = false; if (hiloMotor != null) { synchronized (this) { notifyAll(); } hiloMotor.interrupt(); } publicar(TipoEvento.SIMULACION_DETENIDA, "Detenido"); }
    public void reiniciarDia() { detener(); if (salaEspera != null) salaEspera.reiniciar(); if (cajas != null) cajas.forEach(Caja::reiniciar); generador.reiniciar(); estadisticas.reiniciar(); tiempoReloj = jornadaActual != null ? jornadaActual.getMinutoInicio() : 510; tiempoMotor = 0; fasePrincipalFinalizada = false; faseRezagados = false; publicar(TipoEvento.SIMULACION_REINICIADA, "Reiniciado"); }
    ...
    /**
     * FIX DE ROBUSTEZ: todo el cuerpo va envuelto en try/catch/finally.
     * Si algo revienta de forma inesperada (config invalida, dato raro,
     * lo que sea), el bloque "finally" GARANTIZA que fasePrincipalFinalizada
     * quede en true y corriendo en false -- para que SimuladorMensualService
     * (que espera con esperarCondicion(() -> motor.isFasePrincipalFinalizada()))
     * NUNCA se quede esperando para siempre. Antes, una excepcion silenciosa
     * en este hilo colgaba TODA la simulacion sin ningun aviso.
     */
    private void bucleMotor() {
        try {
            ...
            // ==================== MODO MANUAL (sin cambios de logica) ====================
            double acum = 0.0;
            ...
            System.err.println(">>> [Motor] ERROR inesperado en bucleMotor (dia " + diaSimulado + "): " + t);
            t.printStackTrace();
        } finally {
    ...
    private void bucleMotorReplay() {
        ...
        // NOTA: fasePrincipalFinalizada/corriendo los pone el "finally" de bucleMotor()
    }

    /**
     * FIX DE ROBUSTEZ: mismo try/catch/finally que bucleMotor(). Si algo
     * revienta durante el drenaje de rezagados, "corriendo" siempre queda
     * en false al salir, para que esperarCondicion(() -> !motor.isCorriendo())
     * nunca se quede esperando para siempre.
     */
    private void bucleRezagados() {
        try {
            ...
            System.err.println(">>> [Motor] ERROR inesperado en bucleRezagados (dia " + diaSimulado + "): " + t);
            t.printStackTrace();
        } finally {
            corriendo = false;
        }
    ...
    private void publicar(TipoEvento t, String msg) { EventoSimulacion ev = new EventoSimulacion(t, msg, horaSimulada()); for (var l : listeners) { try { l.accept(ev); } catch (Exception ignored) {} } }
    ...
    public long     getTiempoReloj()            { return tiempoReloj; }
    public long     getTiempoMotor()            { return tiempoMotor; }
    ...
    public JornadaLaboral                 getJornadaActual(){ return jornadaActual; }
}