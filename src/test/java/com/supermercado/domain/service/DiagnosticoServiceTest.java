...
public class DiagnosticoServiceTest {
    ...
    void testDiagnosticoCajaConColaAlta() {
        ...
        for (int i = 0; i < 30; i++) {
            caja.agregarCliente(new com.supermercado.domain.model.Cliente(i, 1));
        }
    ...
    void testGenerarRecomendacionGeneral() {
        List<DiagnosticoService.Diagnostico> diagnosticos = service.diagnosticar(cajas);
        com.supermercado.domain.model.Recomendacion rec = service.generarRecomendacionGeneral(diagnosticos);
        assertNotNull(rec);
